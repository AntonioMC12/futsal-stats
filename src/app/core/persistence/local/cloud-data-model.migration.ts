...
import { Match, matchDateTimestamp } from '../../../shared/models/match';
...
export async function migrateToCloudDataModel(transaction: Transaction): Promise<void> {
  ...
  const strategiesTable = transaction.table<Strategy, string>('strategies');
  const [teams, players, matches, events, strategies] = await Promise.all([
    teamsTable.toArray(),
    playersTable.toArray(),
    matchesTable.toArray(),
    eventsTable.toArray(),
    strategiesTable.toArray(),
  ]);
  ...
  const migratedMatches = matches.map((match): LocalMatchRecord => {
    ...
    const createdAt = timestampOr(match.createdAt, matchDateTimestamp(match.date));
    const updatedAt = timestampOr(match.updatedAt, createdAt);
    const squadPlayerIds = match.squadPlayerIds ?? [];
    const startingLineupPlayerIds = match.startingLineupPlayerIds ?? [];
    return {
      ...match,
      ...
      squadPlayerIds: squadPlayerIds.map((id) => requiredMappedId(playerIds, id, 'Player')),
      startingLineupPlayerIds: startingLineupPlayerIds.map((id) =>
  ...
  const migratedStrategies = strategies.map((strategy): Strategy => ({
    ...strategy,
    teamId: requiredMappedId(teamIds, strategy.teamId, 'Team'),
  }));
  ...
  await transaction.table<Strategy, string>('strategies').bulkPut(migratedStrategies);
}
