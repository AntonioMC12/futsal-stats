...
export class DexieMatchRepository implements MatchRepository {
  ...
  async list(): Promise<Match[]> {
    return (await this.db.matches.orderBy('updatedAt').reverse().toArray()).map(fromStoredMatch);
  }
  ...
  async get(id: string): Promise<Match | undefined> {
    ...
    return record ? fromStoredMatch(record) : undefined;
  }
...
function fromStoredMatch(record: Parameters<typeof fromLocalMatchRecord>[0]): Match {
  return normalizeMatch(fromLocalMatchRecord(record));
}
