...
describe('Dexie cloud data model migration', () => {
  ...
  it('upgrades an existing v3 database without losing tactical strategy ownership', async () => {
    await Dexie.delete('futsal-stats');
    const legacy = createVersion3Database();
    const legacyTeam = {
      id: 'legacy-team',
      name: 'Legacy',
      shortName: 'LEG',
    ...
    let sequence = 0;
    const strategy = createStrategy(legacyTeam.id, [], () => `strategy-${++sequence}`);
    await legacy.table('teams').put(legacyTeam);
    await legacy.table('strategies').put(strategy);
    legacy.close();
...
function createVersion3Database(): Dexie {
