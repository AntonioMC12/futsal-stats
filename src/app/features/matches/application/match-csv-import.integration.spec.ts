...
import {
  MATCH_EVENT_REPOSITORY, MATCH_REPOSITORY, PLAYER_REPOSITORY, TEAM_REPOSITORY,
...
describe('CSV match import integration', () => {
  ...
  beforeEach(async () => {
    ...
    const activeTeam = { id: 'team-target', name: 'Destino', shortName: 'DES', createdAt: 1, updatedAt: 1 };
    TestBed.configureTestingModule({ providers: [
      provideLocalPersistence(), ImportMatchFromCsvUseCase, CsvMatchImportParser,
  ...
  it('runs export → parse → import → repositories without orphan records', async () => {
    const sourcePlayer: Player = { id: 'source-player', teamId: 'source-team', number: 7, name: 'Alex', active: true };
    const sourceMatch: Match = {
      id: 'source-match', teamId: 'source-team',
      homeTeam: { id: 'source-team', name: 'Origen', shortName: 'ORI' },
      awayTeam: { name: 'Rival', shortName: 'RIV' }, date: '2026-09-13', description: 'Copa',
      status: 'finished', currentPeriod: 2, periodCount: 2,
      clock: { ...createMatchClock(), remainingMs: 0 }, squadPlayerIds: [sourcePlayer.id],
      startingLineupPlayerIds: [sourcePlayer.id], createdAt: 1, updatedAt: 2,
    };
    const sourceEvent: MatchEvent = {
      id: 'source-goal', matchId: sourceMatch.id, type: 'GOAL_FOR', scorerPlayerId: sourcePlayer.id,
      lineupPlayerIds: [sourcePlayer.id], scoreBefore: { home: 0, away: 0 }, scoreAfter: { home: 1, away: 0 },
      period: 1, gameClockMs: 600_000, timestamp: 1, sequence: 1, undone: false,
    };
    const csv = serializeMatchCsv(buildMatchStatisticsExport(sourceMatch, [sourceEvent], [sourcePlayer]));
    ...
    expect(savedMatch).toMatchObject({ status: 'finished', source: 'csv-import', description: 'Copa' });
